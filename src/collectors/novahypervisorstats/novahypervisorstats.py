...
class NovaHypervisorStatsCollector(diamond.collector.Collector):
  ...
  def get_default_config_help(self):
    config_help = super(NovaHypervisorStatsCollector, self).get_default_config_help()
    config_help.update({
      ...
      'cpu_allocation_ratio': 'As per nova.conf, the allocation ratio of real to virtual cores',
      'ram_allocation_ratio': 'As per nova.conf, the allocation ratio of real to virtual ram',
    })
    return config_help
  ...
  def collect(self):
    ...
    url = self.config['auth_proto'] + '://' + self.config['auth_url'] + ':5000/v2.0/'
    ...
    if hypervisors:
      ...
      for h in hypervisors:
        ...
        max_vcpus = (int(stats['vcpus']) * float(self.config['cpu_allocation_ratio']))
        vcpus_percent_used = ((stats['vcpus_used']/max_vcpus) * 100)
        self.publish(hostname[0] + '.max_vcpus', max_vcpus)
        self.publish(hostname[0] + '.vcpus_percent_used', vcpus_percent_used)

        max_ram = (int(stats['memory_mb']) * float(self.config['ram_allocation_ratio']))
        ram_percent_used = ((stats['memory_mb_used']/max_ram) * 100)
        self.publish(hostname[0] + '.max_ram', max_ram)
        self.publish(hostname[0] + '.ram_percent_used', ram_percent_used)
