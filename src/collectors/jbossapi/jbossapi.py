...
https://github.com/BrightcoveOS/Diamond/blob/master/src/collectors/mysql/mysql.py
...
TODO:
This code was made to work with the local system 'curl' command, due to difficulties 
getting urllib2 or pycurl to work under the python 2.4 options successfully doing 
SSL Digest Authentication.

Plan is to make this code work with newer versions of python and possibly Requests.
(http://docs.python-requests.org/en/latest/)

If possible, please make future updates backwards compatable to call the local curl as an option.
...
# Uses local system curl until can be made to work with either urllib2, pycurl, or requests (http://docs.python-requests.org/en/latest/)
...
connect_timeout = 4
hosts = wasadmin:XXXXXXXXXX@vifname.domain.net:9443:https, wasadmin:XXXXXXXXXX@vifname2.domain.net:9443:https
...
class JbossApiCollector(diamond.collector.Collector):
    ...
    def __init__(self, config, handlers):
      ...
      if type(self.config['connector_options']) is not list: 
          #self.config['connector_options'] = self.config['connector_options'].split()
          self.config['connector_options'] = [self.config['connector_options']]


    def get_default_config_help(self):

        # Need to update this when done to help explain details when running diamond-setup.
    ...
    def get_stats(self, current_host, current_port, current_proto, current_user, current_pword):
        ...
        if self.config['interface_regex'] != '':
            ...
            # Clean up any possible extra "."'s in the interface, keeps graphite from creating directories
            ...
            if op_type == 'app' and self.config['app_stats'] == 'True':
                ...
                # Grab the pool stats for each Instance 
                    for instance in output['result']['data-source']:
                        for metric in output['result']['data-source'][instance]['statistics']['pool']:
                            metricName = '%s.%s.%s.statistics.pool.%s' % (interface, op_type, instance, metric)
                            metricValue = output['result']['data-source'][instance]['statistics']['pool'][metric]
                ...
                if output:
                    ...
                    #for connector_type in self.config['connector_options']:
                        for metric in web_stats:
                            metricName = '%s.%s.connector.%s.%s' % (interface, op_type, connector_type, metric)
                            metricValue = output['result']['connector'][connector_type][metric]                           
                            #self.log.debug('%s %s', metricName,metricValue)
                            self.publish(metricName,float(metricValue))
            ...
            if op_type == 'jvm':
                if output: 
                    ...
                    # Grab JVM memory pool stats
                        for pool_name in output['result']['type']['memory-pool']['name']:
                            for metric in memory_types:
                                metricName = '%s.%s.%s.%s.%s.%s' % (interface, op_type, 'memory-pool' , pool_name, 'usage', metric)
                                metricValue = output['result']['type']['memory-pool']['name'][pool_name]['usage'][metric]
                                #self.log.debug('%s %s', metricName,metricValue)
                                self.publish(metricName,float(metricValue))

                    # Grab JVM buffer-pool stats
                    if self.config['jvm_buffer_pool_stats'] == 'True':
                        for pool_name in output['result']['type']['buffer-pool']['name']:
                            for metric in buffer_pool_types:
                                metricName = '%s.%s.%s.%s.%s' % (interface, op_type, 'buffer-pool', pool_name, metric)
                                metricValue = output['result']['type']['buffer-pool']['name'][pool_name][metric]
                                #self.log.debug('%s %s', metricName,metricValue)
                                self.publish(metricName,float(metricValue))
                    ...
                    if self.config['jvm_memory_stats'] == 'True':
                        for mem_type in memory_topics:
                            for metric in memory_types:
                                metricName = '%s.%s.%s.%s.%s' % (interface, op_type, 'memory', mem_type , metric)
                                metricValue = output['result']['type']['memory'][mem_type][metric]
                                #self.log.debug('%s %s', metricName,metricValue)
                                self.publish(metricName,float(metricValue))

                    # Grab Garbage collection stats
                    if self.config['jvm_gc_stats'] == 'True':
                        for gc_name in output['result']['type']['garbage-collector']['name']:
                            for metric in gc_types:
                                #metricName = '%s.%s.%s.%s %s' % (self.config['interface'], 'garbage-collector', gc_name , metric,  output['result']['type']['garbage-collector']['name'][gc_name][metric])
                                metricName = '%s.%s.%s.%s.%s' % (interface, op_type, 'garbage-collector', gc_name , metric)
                                metricValue = output['result']['type']['garbage-collector']['name'][gc_name][metric]
                                #self.log.debug('%s %s', metricName,metricValue)
                                self.publish(metricName,float(metricValue))
                    ...
                    if self.config['jvm_thread_stats'] == 'True':
                        for metric in thread_types:
                            metricName = '%s.%s.%s.%s' % (interface, op_type, 'threading' , metric)
                            metricValue = output['result']['type']['threading'][metric]
                            #self.log.debug('%s %s', metricName,metricValue)
                            self.publish(metricName,float(metricValue))
    ...
    def get_data(self, op_type, current_host, current_port, current_proto, current_user, current_pword):
        output = {}
        if op_type == 'app':
            data = '{"operation":"read-resource", "include-runtime":"true", "recursive":"true" , "address":["subsystem","datasources"]}'

        if op_type == 'web':
            data = '{"operation":"read-resource", "include-runtime":"true", "recursive":"true" , "address":["subsystem","web"]}'

        if op_type == 'jvm':
           data = '{"operation":"read-resource", "include-runtime":"true", "recursive":"true" , "address":["core-service","platform-mbean"]}'

        the_cmd = "%s --connect-timeout %s %s %s %s://%s:%s/management --header %s -d '%s' -u %s:%s" % ( self.config['curl_bin'], self.config['connect_timeout'], self.config['ssl_options'], self.config['curl_options'], current_proto, current_host, current_port, header, data, current_user, current_pword)
        #self.log.debug("the command = %s", the_cmd )
