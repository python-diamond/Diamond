...
class FilesCollector(diamond.collector.Collector):
    ...
    def get_default_config_help(self):
        ...
        config_help.update({
            'path':         'Prefix added to all stats collected by this module, a single dot means don''t add prefix',
                ...
                if os.path.isfile(os.path.join(self.config['dir'],fn)):
                    try:
                        ...
                        for line in fh:
                            m = _RE.match(line)
                            if (m):
                                self.publish( m.groups()[0], m.groups()[1]+m.groups()[2], precision=max(0,len(m.groups()[2]) - 1) )
                                found = True
                        fh.close()
