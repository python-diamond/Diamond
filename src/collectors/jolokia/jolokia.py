...
"""
Collects JMX metrics from the Jolokia Agent. Jolokia is an HTTP bridge that provides access to JMX MBeans without the need to write Java code. See the [Reference Guide](http://www.jolokia.org/reference/html/index.html) for more information.

By default, all MBeans will be queried for metrics. All numerical values will be published to Graphite; anything else will be ignored. JolokiaCollector will create a reasonable namespace for each metric based on each MBeans domain and name. e.g) ```java.lang:name=ParNew,type=GarbageCollector``` would become ```java.lang.name_ParNew.type_GarbageCollector```.
...
If desired, JolokiaCollector can be configured to query specific MBeans by providing a list of ```mbeans```. If ```mbeans``` is not provided, all MBeans will be queried for metrics.
...
class JolokiaCollector(diamond.collector.Collector):
    ...
    def get_default_config_help(self):
        ...
        config_help.update({
            'mbeans': "Pipe delimited list of MBeans for which to collect stats. If not provided, all stats will be collected",
            'host': 'Hostname',
            'port': 'Port',
        })
        return config_help
    ...
    def list_request(self):
        try:
            url = "http://%s:%s/%s" % (self.config['host'], self.config['port'], self.LIST_URL)
    ...
    def read_request(self, domain):
        try:
            ...
            url = "http://%s:%s/%s" % (self.config['host'], self.config['port'], url_path)
            response = urllib2.urlopen(url)
            return self.read_json(response)
        except (urllib2.HTTPError, ValueError):
            self.log.error('Unable to read JSON response.')
